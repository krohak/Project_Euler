...
def generateNextActions(prev_action, num_stock):
...
# by updating state
def applyAction(action, num_stock, profit, prices, day):
    ...
    return num_stock, profit


# iterate the tree and store max profit
# across action branches in memo table
def iterateTree(day, action, num_stock, profit, prices):
    ...
    if day < len(prices):
        ...
        actions = generateNextActions(action, num_stock)
        # print('Generate Actions: ', day, action, actions, profit)

        # apply the action and update state
        num_stock_updated, profit_updated = applyAction(action, num_stock, profit, prices, day)
        ...
        for act in actions:    
            ...
            profit_a = iterateTree(day+1, act, num_stock_updated, profit_updated, prices)
...
def iterateTreeCaller(prices):
    ...
    max_profit = 0
    for action in ['b','c']:
        num_stock=0; profit=0
        profit_a = iterateTree(0, action, num_stock, profit, prices)
        if profit_a>max_profit:
            max_profit = profit_a
