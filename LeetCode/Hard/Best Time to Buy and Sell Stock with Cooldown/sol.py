...
def generateNextActions(prev_action, bought):
...
# by updating state
def applyAction(action, bought, prices, day):
...
def iterateTree(day, action, bought, prices):
    ...
    if day < (len(prices)-1):
        ...
        actions = generateNextActions(action, bought)
        
        # apply the action and update state
        bought_updated = applyAction(action, bought, prices, day)

        max_future_profit = 0
        # give actions to updated state
        for act in actions:    
            future_profit = iterateTree(day+1, act, bought_updated, prices)
            # take max across action branches
            if future_profit > max_future_profit:
                max_future_profit = future_profit

        profit_today = calculateProfit(action, prices, day)
        memo[(day, action, bought_updated)] = profit_today + max_future_profit        
        return profit_today + max_future_profit
...
def iterateTreeCaller(prices):
    ...
    for action in ['b','c']:
        bought=0
        profit_a = iterateTree(0, action, bought, prices)
