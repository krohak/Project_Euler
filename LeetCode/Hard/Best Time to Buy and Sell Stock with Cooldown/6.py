...
# by updating state
def applyAction(action, num_stock, bought, prices, day):
    ...
    return num_stock, bought


def calculateProfit(action, prices, day):
...
def iterateTree(day, action, num_stock, bought, prices):

    global memo

    if (day, action, bought) in memo:
        return memo[(day, action, bought)]
    ...
    if day < (len(prices)-1):
        ...
        num_stock_updated, bought_updated = applyAction(action, num_stock, bought, prices, day)
        ...
        for act in actions:    
            ...
            profit_a = iterateTree(day+1, act, num_stock_updated, bought_updated, prices)
        ...
        profit_today = calculateProfit(action, prices, day)
        memo[(day, action, bought_updated)] = profit_today+max_profit

        # print('SUMMARY: ', day, action, actions, max_act, profit_today, max_profit, profit_today+max_profit)
        
        return max_profit+profit_today

    elif day == (len(prices)-1):
        profit_last = calculateProfit(action, prices, day)
        # print('SUMMARY LAST: ', day, action, profit_last)
        return profit_last
...
def iterateTreeCaller(prices):
    ...
    max_profit = 0
    for action in ['b','c']:
        num_stock=0; bought=0
        profit_a = iterateTree(0, action, num_stock, bought, prices)
        if profit_a>max_profit:
            max_profit = profit_a
