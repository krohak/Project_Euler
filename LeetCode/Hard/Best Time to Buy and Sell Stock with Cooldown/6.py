...
def updatePrice(action, prices, day):
...
def iterateTree(day, action, num_stock, profit, bought, prices):
    ...
    if (day, profit, action) in memo:
        return memo[(day, profit, action)]
    ...
    if day < (len(prices)-1):
        ...
        for act in actions:    
            ...
            profit_a = iterateTree(day+1, act, num_stock_updated, profit, bought_updated, prices)
            # profit_a+=profit
        ...
        price_updated = updatePrice(action, prices, day)
        memo[(day,price_updated, action)] = max_profit+price_updated
        return max_profit+price_updated

    elif day == (len(prices)-1):
        return updatePrice(action, prices, day)
